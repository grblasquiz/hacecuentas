...
| --- | --- | --- |
| `sitemap-state.json` | **Estado anti-churn del sitemap.** Mapa `URL → lastmod previo` que usa el *rollout cap* de `scripts/generate-sitemap.ts` para limitar cuánto sube el `lastmod` de cada URL entre deploys. | **NO.** Sin él, un build fresco cae a `xml-bootstrap` o `fresh` y puede re-estampar `lastmod` de miles de URLs el mismo día → patrón de "sitemap sucio" que Google penaliza (regla #1 de `CLAUDE.md`). |
