...
const jsonData = readFileSync('public/datos/salario-minimo-mexico-2026.json', 'utf8');
const csvData = readFileSync('public/datos/salario-minimo-mexico-2026.csv', 'utf8');
...
describe('SEO canónico del salario mínimo de México', () => {
  it('responde la consulta y el valor directamente en title, H1 y descripción', () => {
    expect(source).toContain("const title = 'Salario mínimo México 2026: $315.04 diarios y $9,582.47 al mes'");
    ...
    expect(source).toContain('<strong>$9,582.47 al mes</strong>');
  });
  ...
  it('publica el equivalente mensual anualizado también en los datos descargables', () => {
    expect(jsonData).toContain('"general_mensual": 9582.47');
    expect(jsonData).toContain('"zlfn_mensual": 13407.30');
    expect(csvData).toContain('Salario minimo general (mensual),9582.47');
    expect(csvData).toContain('Salario minimo ZLFN (mensual),13407.30');
  });
