...
const COUNTRY_SUFFIX = [['espana','es'],['colombia','co'],['ecuador','ec'],['peru','pe'],['chile','cl'],['mexico','mx'],['argentina','']];
function resolveCountryFallback(pathname) {
  const m = pathname.match(/^\\/(mx|co|cl|pe|ec|es|ve|py|uy|do)\\/(calculadora-[a-z0-9-]+)$/);
  if (m) {
    for (const [suffix, cc] of COUNTRY_SUFFIX) {
      if (cc === m[1] || !m[2].includes('-' + suffix)) continue;
      const candidate = (cc ? '/' + cc : '') + '/' + m[2];
      const hit = REDIRECT_MAP[candidate];
      if (hit) return hit.d;
      if (cc === 'es' || cc === 'co' || cc === 'ec' || cc === 'pe' || cc === 'cl') return candidate;
    }
    return null;
  }
  if (/^\\/[a-z0-9-]+$/.test(pathname)) {
    const hit = REDIRECT_MAP['/en' + pathname];
    if (hit) return hit.d;
  }
  return null;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (APEX_HOSTS.has(url.hostname)) {
      ...
      // 4b) Fallback para prefijo de país equivocado y slugs EN sin prefijo.
      // Bing sigue pidiendo '/mx/calculadora-...-espana-...' (generadas con el
      // prefijo mal) y '/tip-calculator' (EN en raíz). Si existe una regla para
      // '/<cc>/<slug>' o '/en/<slug>', mandamos a su destino final (sin cadena).
      const fallbackTarget = resolveCountryFallback(url.pathname);
      if (fallbackTarget) {
        return Response.redirect(\`https://hacecuentas.com\${fallbackTarget}\${url.search}\`, 301);
      }
