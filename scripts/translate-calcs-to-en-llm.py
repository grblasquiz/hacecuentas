...
TARGETS_PER_CAT = {
    'salud': 110, 'matematica': 80, 'finanzas': 55, 'cocina': 50,
    'deportes': 45, 'mascotas': 40, 'ciencia': 30, 'construccion': 30,
    'educacion': 20, 'tecnologia': 20, 'jardineria': 10, 'automotor': 10,
}
...
PROMPT_TEMPLATE = """You are translating a calculator website page from Argentine Spanish to native US English, optimized for search engine ranking.
...
3. TRANSLATE naturally to native US English (NOT word-for-word, use idiomatic SEO phrasing):
   - "title": max 62 chars total. End with " 2026 | Hacé Cuentas" OR " | Hacé Cuentas". Include a keyword users actually search.
   - "h1": natural headline (no keyword stuffing), usually different phrasing than title
   - "description": meta description, max 158 chars, compelling for CTR
   - "intro", "keyTakeaway", "useCases[]": native, specific, with real numbers and examples
   - "explanation": natural English prose with proper headings (## and ###)
   - "faq[]": replace with 7-10 questions that REAL English speakers search (e.g., "how much water should my dog drink", not translated Spanish questions). Use US terminology.
   - fields[].label, fields[].help, fields[].options[].label: clear English labels
   - outputs[].label: concise English labels
   - example: translate title/steps/result to English
4. REPLACE seoKeywords with 8-14 ENGLISH keywords real users search (long-tail + short-tail mix, US terminology).
5. ADD these fields:
   - "slug": remove "calculadora-" prefix from original Spanish slug, lowercase-kebab. Keep descriptive.
   - "esSlug": set to the original Spanish slug exactly as given (for hreflang alternate linking)
   - "audience": "global"
   - "relatedSlugs": []
6. Units: preserve metric if in source. Don't force imperial conversion.
7. Don't hallucinate fields that didn't exist. Don't add optional ones not in input.
8. If a field is missing or empty in input, omit it from output (don't invent).
