...
Genera N calcs específicas para España. Output:
  src/content/calcs-mx/{slug}.json   — con audience=MX, lang=es-MX
...
SPECS_FILE = ROOT / "scripts" / "calcs-mexico-specs.json"
CALCS_DIR = ROOT / "src" / "content" / "calcs-mx"
FORMULAS_DIR = ROOT / "src" / "lib" / "formulas"
STATE_FILE = ROOT / "scripts" / ".batch-mexico-state.json"

MODEL = "claude-haiku-4-5"
MAX_TOKENS = 16000

SYSTEM_PROMPT = """Sos un generador de calculadoras para hacecuentas.com, sección México.

Tu trabajo: producir el JSON spec + el código TypeScript de la fórmula para una calc nueva ESPECÍFICA PARA MÉXICO.

# REGLA CRÍTICA: idioma y referencias

- TODO el output en ESPAÑOL DE MÉXICO (tú, NO vos, NO tú/usted neutro).
- Usar "tú" o construcciones impersonales — NUNCA "vos".
- Moneda: pesos mexicanos (\$), formato \,234.56.
- Referencias OFICIALES España: SAT, IMSS, INFONAVIT, CONDUSEF, Banxico, INEGI, Profeco, CFE, IMSS.
- Tramos ISR México 1.92-35%, IVA 16% (8% frontera), IMSS, RFC/CURP, estados (32).
- Datos vigentes 2026 México.
...
```
{
  ...
  "description": "Meta SEO 120-160 chars con cifra concreta México",
  "category": "...",                              # del spec
  "audience": "MX",                               # FIJO: MX (México)
  "icon": "emoji",                                # del spec
  "formulaId": "<slug-sin-prefix-calculadora>",
  "intro": "200-400 chars, español México. Markdown light: **bold**.",
  ...
  "faq": [...],                                   # 7-10 preguntas concretas México
  "sources": [                                    # 2-5 fuentes oficiales reales México
    {"name": "SAT - ISR", "url": "https://www.sat.gob.mx/...", "publisher": "SAT", "date": "2026"}
  ],
  "dataUpdate": {
...
export function compute(i: Inputs): Outputs {
  // lógica con constantes 2026 México nombradas + comentario fuente
  // mensajes en strings TS en español de México
...
async def generate_one(client, spec, sem):
    slug = spec["slug"]
    user = (
        f"IMPORTANTE: español de España. Datos SAT/IRPF 2026.\n\n"
        f"slug: {slug}\n"
        f"h1: {spec['h1']}\n"
        ...
        f"icon: {spec['icon']}\n"
        f"brief: {spec['brief']}\n\n"
        f"Devolvé solo JSON con `json` y `formula_ts`. TODO en español México."
    )
    async with sem:
        try:
...
async def main():
    ...
    print(f"[batch-mexico] {len(specs)} calcs con {MODEL}, conc={args.concurrency}", file=sys.stderr)
    ...
    save_state(state)
    print(f"\n[batch-mexico] done: {ok} ok / {err} err", file=sys.stderr)
