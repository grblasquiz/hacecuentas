...
CALCS_EN_DIR = ROOT / 'src/content/calcs-pt'
OUT_FILE = ROOT / 'src/lib/formula-strings-i18n-pt.json'
...
PROMPT_TEMPLATE = """You are a professional translator. Translate the following Argentine Spanish strings (from a calculator website) to NEUTRAL Portuguese that works for BOTH Brazil and Portugal.
...
2. Use neutral Portuguese: "você" (not "tu"), avoid Brazilian slang and Portugal-specific terms.
3. Preserve numbers, units, and punctuation structure EXACTLY.
4. Natural idiomatic Portuguese (not word-for-word). "por día" → "por dia", "recomendable" → "recomendável".
5. Keep the same level of formality/brevity as source.
6. If the string contains embedded numbers or measurements, preserve them EXACTLY.

INPUT (JSON array of Spanish strings):
{strings}

OUTPUT FORMAT (JSON object, key = original ES string, value = PT translation):
{{
  "original spanish string 1": "portuguese translation 1",
  "original spanish string 2": "portuguese translation 2"
}}
