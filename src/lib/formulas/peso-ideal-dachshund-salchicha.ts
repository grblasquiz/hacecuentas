/**
 * Calculadora de Peso Ideal del Dachshund (Salchicha)
 ...
 * dachshund (salchicha) de 2 meses"). Ver src/lib/formulas/_puppy-growth.ts.
 */
...
const TAMANO = tamanoPorPeso((7 + 14 + 7 + 14) / 4);

export function pesoIdealDachshundSalchicha(inputs: Inputs): Outputs {
  ...
  if (meses !== null) {
    ...
    resumen = `A los ${meses} meses un Dachshund (Salchicha) ${sexoTxt} pesa entre ${fmtKg(p.min)} y ${fmtKg(p.max)} kg (${p.porcentaje}% de su peso adulto).`;
    insightTitle = `Tu cachorro a los ${meses} meses`;
    insightText = `${sexoArt} Dachshund (Salchicha) ${sexoTxt} de **${meses} meses** debería pesar entre **${fmtKg(p.min)} y ${fmtKg(p.max)} kg** — cerca del **${p.porcentaje}%** de su peso adulto. De grande va a rondar los **${fmtKg(adultoMin)} a ${fmtKg(adultoMax)} kg**, y termina de crecer alrededor de los **${p.cierreMeses} meses**. Que esté algo por debajo o por encima no alarma: lo que importa es que la curva suba parejo y que las costillas se palpen sin apretar.`;
    insightTone = 'neutral';
  } else if (edad === 'senior') {
    min = adultoMin * 0.9;
    max = adultoMax;
    resumen = `Senior: podría bajar un 5-10% del peso de adulto. Rango ${fmtKg(min)}-${fmtKg(max)} kg.`;
    insightTitle = 'Tu Dachshund (Salchicha) senior';
    insightText = `${sexoArt} Dachshund (Salchicha) ${sexoTxt} senior suele pesar entre **${fmtKg(min)} y ${fmtKg(max)} kg**, algo menos que en su adultez por la pérdida natural de masa muscular. Si baja de golpe o pierde tono muscular notorio, conviene un control veterinario.`;
    insightTone = 'warn';
  } else {
    resumen = `${sexo === 'macho' ? 'Macho' : 'Hembra'} ${contextura}: peso ideal ${fmtKg(min)}-${fmtKg(max)} kg`;
    insightTitle = 'Tu Dachshund (Salchicha) en su peso';
    insightText = `${sexoArt} Dachshund (Salchicha) ${sexoTxt} de contextura ${contextura} debería pesar entre **${fmtKg(min)} y ${fmtKg(max)} kg** (promedio **${fmtKg(adultoProm)} kg**). Guiate también por el Body Condition Score: costillas palpables sin apretar y cintura visible desde arriba valen más que el número de la balanza.`;
  }
  ...
  const chartCurva = {
    ...
    ariaLabel: `Curva de crecimiento del Dachshund (Salchicha) ${sexoTxt}: de ${fmtKg(serie[0].min)}-${fmtKg(serie[0].max)} kg a los ${serie[0].meses} meses hasta ${fmtKg(adultoMin)}-${fmtKg(adultoMax)} kg de adulto`,
  };
