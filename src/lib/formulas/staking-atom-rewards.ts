...
export function stakingAtomRewards(i: Inputs): Outputs {
  ...
  if (!amt || amt <= 0) throw new Error('Ingresá la cantidad de ATOM');
  ...
  const insight = {
    ...
    text: commission > 0.1
      ? `En ${months} meses ganás **${netTokens.toFixed(2)} ATOM netos** ($${usd.toFixed(2)} USD), pero la comisión del ${(commission*100).toFixed(1)}% del validador te resta **${comisionTokens.toFixed(2)} ATOM**. Elegí un validador con comisión más baja para quedarte con más.`
      : `Stakeando ${amt} ATOM sumás **${netTokens.toFixed(2)} ATOM netos** ($${usd.toFixed(2)} USD) en ${months} meses, un **${effectiveApy.toFixed(2)}% efectivo anual** tras comisión. Recordá el período de unbonding (~21 días) antes de poder mover los tokens.`,
