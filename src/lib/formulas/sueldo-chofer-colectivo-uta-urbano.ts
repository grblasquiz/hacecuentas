...
export function sueldoChoferColectivoUtaUrbano(i: Inputs): Outputs {
  ...
  const insight={
    ...
    text:`Sobre un bruto de **${fmtAr(bruto)}** se descuentan **${fmtAr(descTotal)}** (${pctDesc.toFixed(0)}%) en aportes y el neto urbano queda en **${fmtAr(neto)}**. La antigüedad de ${antig} año${antig===1?'':'s'} suma **${fmtAr(plusAntig)}**; este cálculo es por el básico UTA, sin contar viáticos ni horas extra.`,
