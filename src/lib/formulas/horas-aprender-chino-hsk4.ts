/** ¿Cuántas horas para llegar a chino mandarín HSK 4? */
...
export function horasAprenderChinoHsk4(i: Inputs): Outputs {
  ...
  const ritmo =
    mes <= 12
      ...
      : mes <= 24
        ...
        : `A **${hWk} h/semana** te llevaría **${mes} meses**; el chino es **Cat IV del FSI** (de los más difíciles para hispanohablantes, sobre todo por los caracteres y los tonos), así que subir el ritmo o sumar inmersión acorta mucho el camino.`;

  const segTop = Math.max(Math.ceil(mes * 1.15), 30);
  const _insight = {
    title: 'Tu camino al chino HSK 4',
  ...
  const _chart = {
    ...
    ariaLabel: `Horizonte de estudio: ${mes} meses para llegar a chino HSK 4`,
  };
