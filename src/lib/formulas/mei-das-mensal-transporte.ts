 ...
 *  Valor 2026: INSS 12% do salário mínimo (R$ 194,52) + ICMS R$ 1,00 = R$ 195,52.
...
export function meiDasMensalTransporte(i: Inputs): Outputs {
  ...
  const DAS_TRANSPORTE = DAS_MEI.transportador;
  const INSS = DAS_MEI.inss12;
  ...
  const composicao = `INSS 12% (do salário mínimo) R$ ${INSS.toFixed(2)} + ICMS R$ ${ICMS.toFixed(2)} = R$ ${DAS_TRANSPORTE.toFixed(2)}`;
  ...
  const _chart = {
    ...
    ariaLabel: `Composição do DAS mensal do transportador de R$ ${DAS_TRANSPORTE.toFixed(2)}: INSS R$ ${INSS.toFixed(2)}, e ICMS R$ ${ICMS.toFixed(2)}.`,
  };
