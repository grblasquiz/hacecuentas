...
export function rmPressBancaEstimador(i: Inputs): Outputs {
  ...
  if (r <= 1) {
    ...
    insightText = __lang === 'pt'
      ? `Com 1 rep, **${p} kg já é seu 1RM real** no supino. Para força treine perto de **${fuerza} kg** (85%); para hipertrofia, **${hipertrofiaLo}–${hipertrofiaHi} kg**.`
      : `Con 1 rep, **${p} kg ya es tu 1RM real** en press de banca. Para fuerza entrená cerca de **${fuerza} kg** (85%); para hipertrofia, **${hipertrofiaLo}–${hipertrofiaHi} kg**.`;
  } else if (r <= 6) {
    insightTone = 'good';
    insightText = __lang === 'pt'
      ? `De **${p} kg × ${r} reps**, seu 1RM de supino é **${rmF} kg** — estimativa confiável. Treine força a **${fuerza} kg** ou hipertrofia a **${hipertrofiaLo}–${hipertrofiaHi} kg**.`
      : `Desde **${p} kg × ${r} reps**, tu 1RM de press de banca es **${rmF} kg** — estimación confiable. Entrená fuerza a **${fuerza} kg** o hipertrofia a **${hipertrofiaLo}–${hipertrofiaHi} kg**.`;
  } else {
    insightTone = 'warn';
    insightText = __lang === 'pt'
  ...
  const _insight = {
    title: __lang === 'pt' ? 'Seu 1RM no supino' : 'Tu 1RM en press de banca',
    text: insightText,
    tone: insightTone,
    icon: '🏋️',
  };
