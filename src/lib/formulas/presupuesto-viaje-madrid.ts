/**
 * Calculadora de Presupuesto de Viaje a Madrid
 */
export interface PresupuestoViajeMadridInputs {
  dias: number;
  nivelHotel: string;
  personas: number;
  incluirVuelo: string;
}
export interface PresupuestoViajeMadridOutputs {
...
const HOTEL: Record<string, number> = { bajo: 60, medio: 120, alto: 250 };
const FOOD = 45;
const TRANS = 10;
const ACTS = 25;
const VUELO_EST = 900; // USD promedio desde Latam
export function presupuestoViajeMadrid(i: PresupuestoViajeMadridInputs): PresupuestoViajeMadridOutputs {
  ...
  const _insight = {
    title: 'Tu presupuesto para Madrid',
    text: vuelo
      ? `El viaje completo da **USD ${fmt(total)}** para ${personas} ${personas === 1 ? 'persona' : 'personas'} por ${dias} ${dias === 1 ? 'día' : 'días'}. El vuelo se lleva **USD ${fmt(vueloTotal)}** (${Math.round(vueloShare * 100)}%); en destino gastás **USD ${perDiem}** por persona y día.`
      : `Sin vuelos, gastás **USD ${fmt(total)}** en destino: **USD ${perDiem}** por persona y día. Madrid es de las capitales europeas más equilibradas en precio: comés y te movés barato comparado con Londres o París.`,
  ...
  const _chart = {
    ...
    centerLabel: 'Total',
    ariaLabel: `Desglose del presupuesto de viaje a Madrid: total USD ${fmt(total)}`
  };
  return {
