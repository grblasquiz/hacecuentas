...
export function crecimientoPercentilEdadAlturaPesoOms(i: Inputs): Outputs {
  ...
  const ambosNormal = pctPeso.includes('normal') && pctTalla.includes('normal');
  const _insight = {
    title: 'Lectura de los percentiles',
    text: ambosNormal
      ? `A los **${m} meses**, peso (**${pctPeso}**) y talla (**${pctTalla}**) caen en la franja media de las curvas OMS. Es una aproximación: confirmá siempre con el control pediátrico.`
      : `A los **${m} meses**, el peso se ubica en **${pctPeso}** y la talla en **${pctTalla}**. Cualquier valor fuera de P25-75 conviene revisarlo con el pediatra; esto es solo una estimación orientativa.`,
    tone: ambosNormal ? 'good' : 'neutral',
    icon: '👶',
  };
  return { percentilPeso:pctPeso, percentilTalla:pctTalla, interpretacion:'Aproximación. Control pediátrico oficial siempre.', _insight };
}
