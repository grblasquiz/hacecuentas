...
export function pesoIdealBeagle(inputs: Inputs): Outputs {
  ...
  if (edad === 'cachorro') {
    insightText = `Tu Beagle cachorro todavía está creciendo: de adulto debería rondar **${minR}–${maxR} kg**. Pesalo cada par de semanas y seguí la curva de tu veterinario, sin forzar la comida.`;
  } else if (edad === 'senior') {
    insightText = `En un Beagle senior es normal perder algo de masa: el rango esperado baja a **${minR}–${maxR} kg**. Vigilá que no caiga por debajo ni gane de más, porque la raza tiende a la obesidad.`;
  } else {
    insightText = `Un Beagle ${sexoTxt} de contextura ${contextura} debería pesar entre **${minR} y ${maxR} kg** (centro ideal **${promR} kg**). Es una raza muy propensa al sobrepeso, así que conviene mantenerla cerca de ese rango.`;
  }
  ...
  return {
    ...
    _chart: {
      ...
      ariaLabel: `Escala de peso del Beagle: zona ideal ${minR} a ${maxR} kg, con el centro en ${promR} kg`,
    },
  };
}
