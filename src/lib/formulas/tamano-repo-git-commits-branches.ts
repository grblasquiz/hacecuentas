...
export function tamanoRepoGitCommitsBranches(i: Inputs): Outputs {
  ...
  return {
    estimado:`${mb.toFixed(1)} MB`, consejo:cons, resumen:`Repo estimado: ${mb.toFixed(0)} MB. ${cons}.`,
    _insight: {
      title: pesado ? 'Repo pesado' : medio ? 'Empieza a pesar' : 'Tamaño saludable',
      text: pesado
        ? `Con **${a.toLocaleString('es-AR')} archivos** y **${c.toLocaleString('es-AR')} commits** el repo ronda los **${mb.toFixed(0)} MB**. A este tamaño los \`clone\` y \`fetch\` se vuelven lentos: mové binarios y archivos grandes a **Git LFS** o partí el monorepo.`
        : medio
