...
export function horasAprenderJaponesN3(i: Inputs): Outputs {
  ...
  const ritmo =
    mes <= 12
      ? `A **${hWk} h/semana** llegás al N3 en poco más de un año: un ritmo intenso pero realista.`
      : mes <= 24
        ? `Con **${hWk} h/semana** es un proyecto de **${mes} meses**: pensalo como una maratón, no un sprint.`
        : `A **${hWk} h/semana** te llevaría **${mes} meses**; el japonés es **Cat IV del FSI** (de los más difíciles para hispanohablantes, sobre todo por los kanji), así que subir el ritmo o sumar inmersión acorta mucho el camino.`;

  const segTop = Math.max(Math.ceil(mes * 1.15), 30);
  const _insight = {
    title: 'Tu camino al japonés N3',
  ...
  const _chart = {
    ...
    ariaLabel: `Horizonte de estudio: ${mes} meses para llegar a japonés N3`,
  };
