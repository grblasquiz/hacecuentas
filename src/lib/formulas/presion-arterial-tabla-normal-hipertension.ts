...
export function presionArterialTablaNormalHipertension(i: Inputs): Outputs {
  ...
  const _insight = __lang === 'en'
    ? { title: 'Result', text: `With **${v1}** and **${v2}**, the result is **${r.toFixed(2)}**.`, tone: 'neutral', icon: '🩺' }
    : { title: 'Resultado', text: `Con **${v1}** y **${v2}**, el resultado es **${r.toFixed(2)}**.`, tone: 'neutral', icon: '🩺' };
