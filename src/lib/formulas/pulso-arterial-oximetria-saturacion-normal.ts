...
export function pulsoArterialOximetriaSaturacionNormal(i: Inputs): Outputs {
  ...
  const _insight = {
    ...
    text: __lang === 'en'
      ? `With **${v1}** and **${v2}**, the result is **${r.toFixed(2)}**. Use it as a reference value for your case.`
      : `Con **${v1}** y **${v2}**, el resultado es **${r.toFixed(2)}**. Tomalo como valor de referencia para tu caso.`,
    tone: 'neutral',
    icon: '🩺',
  };
