...
export function tiempoDigestionAlimentosEstomago(i: Inputs): Outputs {
  ...
  const _insight = __lang === 'en'
    ? { title: 'Your result', text: `With the values entered (${v1} and ${v2}), the result is **${r.toFixed(2)}**.`, tone: 'neutral', icon: '🍽️' }
    : { title: 'Tu resultado', text: `Con los valores ingresados (${v1} y ${v2}), el resultado es **${r.toFixed(2)}**.`, tone: 'neutral', icon: '🍽️' };
