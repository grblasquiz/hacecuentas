...
export function factorPotenciaCorregir(i: FactorPotenciaCorregirInputs): FactorPotenciaCorregirOutputs {
  ...
  return {
    ...
    resumen: __lang === 'en'
      ? `Capacitor of ${(cFarads * 1e6).toFixed(0)} µF corrects cos(φ) from ${c1} to ${c2}. Apparent load reduction: ${((s1-s2)/1000).toFixed(2)} kVA.`
      : `Capacitor de ${(cFarads * 1e6).toFixed(0)} µF corrige cos(φ) de ${c1} a ${c2}. Reducción de carga aparente: ${((s1-s2)/1000).toFixed(2)} kVA.`
  };
}
