...
export function millasAviancaLifemiles(i: MillasAviancaLifemilesInputs): MillasAviancaLifemilesOutputs {
  ...
  const _insight = cab === 'business'
    ? { title: 'Canje premium', text: `Volar a **${dn}** en **business** con LifeMiles cuesta **${millasFmt} millas** (valor aprox. **USD ${valor.toFixed(0)}**). LifeMiles no cobra fuel surcharges, así que el canje en cabina premium rinde.`, tone: 'good', icon: '🛫' }
    : { title: `Canje a ${dn}`, text: `Llegar a **${dn}** en ${cabNombre} sale **${millasFmt} millas** (valor aprox. **USD ${valor.toFixed(0)}**). Sumá **USD 160-300** de tasas y fees aparte.`, tone: 'neutral', icon: '✈️' };
  return {
