...
export function horasAprenderPortuguesB2(i: Inputs): Outputs {
  ...
  const ritmo =
    mes <= 12
      ? `A **${hWk} h/semana** el B2 llega rápido: el portugués es **Cat I del FSI** y de los idiomas más cercanos al español, así que partís con mucha ventaja.`
      : mes <= 24
        ? `Con **${hWk} h/semana** son **${mes} meses**; al ser **Cat I del FSI** (de los más cercanos al español), con un poco más de ritmo el B2 llega antes.`
        : `A **${hWk} h/semana** te llevaría **${mes} meses**; como el portugués es **Cat I del FSI** (muy cercano al español), subir las horas semanales recorta bastante el plazo.`;

  const segTop = Math.max(Math.ceil(mes * 1.15), 30);
  const _insight = {
    title: 'Tu camino al portugués B2',
  ...
  const _chart = {
    ...
    ariaLabel: `Horizonte de estudio: ${mes} meses para llegar a portugués B2`,
  };
