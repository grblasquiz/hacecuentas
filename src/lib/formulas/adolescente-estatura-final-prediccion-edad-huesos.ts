...
export function adolescenteEstaturaFinalPrediccionEdadHuesos(i: Inputs): Outputs {
  ...
  const T = ({
    es: { observacion: 'Aproximación. Nutrición, genética y pubertad afectan.' },
    en: { observacion: 'Estimate only. Nutrition, genetics and puberty all play a role.' },
  } as const)[__lang];
  ...
  return { estaturaFinal:`${Math.round(prom)} cm`, rango:`${Math.round(prom-5)}-${Math.round(prom+5)} cm (±5 cm)`, observacion:T.observacion };
}
