...
export function stakingSolRewards(i: Inputs): Outputs {
  ...
  if (!amt || amt <= 0) throw new Error('Ingresá la cantidad de SOL');
  ...
  const insight = {
    ...
    text: highFee
      ? `A ${(apy*100).toFixed(2)}% APY ganás **${netTokens.toFixed(2)} SOL** (~$${usd.toFixed(0)}) en ${months} meses, pero el validador se queda con el **${(commission*100).toFixed(1)}%** de las recompensas. Buscá un validador con comisión más baja para retener más rendimiento.`
      : `Stakeando ${amt} SOL a ${(apy*100).toFixed(2)}% APY sumás **${netTokens.toFixed(2)} SOL netos** (~$${usd.toFixed(0)}) en ${months} meses, el **${rewardShare.toFixed(1)}%** de tu valor final. APY efectivo (con interés compuesto diario): **${effectiveApy.toFixed(2)}%**. Recordá que el valor en USD depende del precio del token.`,
