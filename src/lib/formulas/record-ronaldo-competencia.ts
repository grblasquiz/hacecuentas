...
export function recordRonaldoCompetencia(i: Inputs): Outputs {
  ...
  const insight = {
    ...
    text: esTotal
      ? `En toda su carrera oficial Cristiano lleva **${fila.goles} goles** en **${fila.partidos} partidos**, un promedio de **${ppp.toFixed(2)} goles por partido** desde ${fila.periodo.replace('–', ' a ')}.`
  ...
  const chart = (!esTotal && resto > 0) ? {
    ...
    ariaLabel: `Goles de Cristiano en ${fila.label} (${fila.goles}) frente al resto de su carrera (${resto}), sobre un total de ${career}.`,
  } : undefined;
