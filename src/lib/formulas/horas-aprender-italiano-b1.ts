...
export function horasAprenderItalianoB1(i: Inputs): Outputs {
  ...
  const ritmo =
    mes <= 12
      ? `A **${hWk} h/semana** el B1 está al alcance: el italiano es **Cat I del FSI**, uno de los más cercanos al español, y B1 ya te habilita a conversar.`
      : mes <= 24
        ? `Con **${hWk} h/semana** son **${mes} meses**; al ser **Cat I del FSI** (muy cercano al español), con un poco más de ritmo el B1 llega antes.`
        : `A **${hWk} h/semana** te llevaría **${mes} meses**; como el italiano es **Cat I del FSI** (muy cercano al español), subir las horas semanales acorta mucho el plazo.`;

  const segTop = Math.max(Math.ceil(mes * 1.15), 30);
  const _insight = {
    title: 'Tu camino al italiano B1',
  ...
  const _chart = {
    ...
    ariaLabel: `Horizonte de estudio: ${mes} meses para llegar a italiano B1`,
  };
