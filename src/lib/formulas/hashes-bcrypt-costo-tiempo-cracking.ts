...
export function hashesBcryptCostoTiempoCracking(i: Inputs): Outputs {
  ...
  // Reference: ~65 ms per hash at cost=10 on a modern single CPU core
  // (measured benchmark, e.g. bcrypt.js on a 2024-era x86 core). Each +1 doubles.
  const base=65; const ms=base*Math.pow(2,c-10);
  const hs=1000/ms;
