...
export function stakingDotRewards(i: Inputs): Outputs {
  ...
  if (!amt || amt <= 0) throw new Error('Ingresá la cantidad de DOT');
  ...
  const insight = {
    ...
    text: highFee
      ? `A ${(apy*100).toFixed(2)}% APY ganás **${netTokens.toFixed(2)} DOT** (~$${usd.toFixed(0)}) en ${months} meses, pero el validador se queda con el **${(commission*100).toFixed(1)}%** de las recompensas. Buscá un nominador con comisión más baja para retener más rendimiento.`
      : `Stakeando ${amt} DOT a ${(apy*100).toFixed(2)}% APY sumás **${netTokens.toFixed(2)} DOT netos** (~$${usd.toFixed(0)}) en ${months} meses, el **${rewardShare.toFixed(1)}%** de tu valor final. APY efectivo (con interés compuesto diario): **${effectiveApy.toFixed(2)}%**. Recordá que el valor en USD depende del precio del token.`,
  ...
  return {
    ...
    _chart: chart,
    explicacion: `Stakeando ${amt} DOT al ${(apy*100).toFixed(2)}% APY durante ${months} meses (comisión ${(commission*100).toFixed(1)}%): ganás ${netTokens.toFixed(2)} DOT netos = $${usd.toFixed(2)} USD.`,
    _insight: insight,
  };
}
