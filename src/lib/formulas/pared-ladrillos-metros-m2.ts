...
export function paredLadrillosMetrosM2(i: Inputs): Outputs {
  ...
  const _insight = {
    ...
    text: `Para **${m} m²** de pared necesitás **${fmt(total)} ladrillos ${i.tipo}**: ${fmt(sinDesperdicio)} para el muro más ${fmt(desperdicio)} de reserva por roturas y cortes (+8%). Comprá por bulto cerrado para no quedarte corto.`,
    tone: 'neutral',
    icon: '🧱',
  };
  const _chart = {
    ...
    slices: [
      ...
      { label: 'Desperdicio (+8%)', value: desperdicio },
    ],
    prefix: '',
  ...
  return { cantidad: total.toString(), resumen: `${total} ladrillos ${i.tipo} para ${m} m² (+8% desperdicio).`, _insight, _chart };
}
