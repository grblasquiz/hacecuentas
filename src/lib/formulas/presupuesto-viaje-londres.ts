/**
 * Calculadora de Presupuesto de Viaje a Londres
 */
export interface PresupuestoViajeLondresInputs {
  dias: number;
  nivelHotel: string;
  personas: number;
  incluirVuelo: string;
}
export interface PresupuestoViajeLondresOutputs {
...
const HOTEL: Record<string, number> = { bajo: 100, medio: 200, alto: 400 };
const FOOD = 70;
const TRANS = 18;
const ACTS = 45;
const VUELO_EST = 900; // USD promedio desde Latam
export function presupuestoViajeLondres(i: PresupuestoViajeLondresInputs): PresupuestoViajeLondresOutputs {
  ...
  const _insight = {
    title: 'Tu presupuesto para Londres',
    text: vuelo
      ? `El viaje completo da **USD ${fmt(total)}** para ${personas} ${personas === 1 ? 'persona' : 'personas'} por ${dias} ${dias === 1 ? 'día' : 'días'}. El vuelo aporta **USD ${fmt(vueloTotal)}** (${Math.round(vueloShare * 100)}%) y en destino gastás **USD ${perDiem}** por persona y día.`
      : `Sin vuelos, gastás **USD ${fmt(total)}** en destino: **USD ${perDiem}** por persona y día. Londres es de las capitales más caras de Europa, sobre todo hotel y comida.`,
    tone: 'warn',
    icon: '🇬🇧'
  };
  const slices = [
  ...
  const _chart = {
    ...
    centerLabel: 'Total',
    ariaLabel: `Desglose del presupuesto de viaje a Londres: total USD ${fmt(total)}`
  };
  return {
