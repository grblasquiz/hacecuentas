...
export function alquilerConOpcionACompraLeasingInmueble(i: Inputs): Outputs {
  ...
  const a=Number(i.alquilerMensual)||0; const m=Number(i.mesesPago)||0; const v=Number(i.valorInmueble)||0; const p=((Number.isFinite(Number(i.pctDescontable)) ? Number(i.pctDescontable) : 30))/100;
  const totAlq=a*m;
  const acum=totAlq*p;
  const falta=v-acum;
