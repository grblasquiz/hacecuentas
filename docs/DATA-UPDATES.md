...
| biannual  | 8     | ⏳ pendiente (todas son auto-llm) |
...
- **[`update-data-biannual.yml`](../.github/workflows/update-data-biannual.yml)** — cron 15 enero + 15 julio. Por ahora solo crea un **issue** con checklist de revisión manual (hasta que implementemos auto-llm).
...
- **`auto-scrape`** — hay que scrapear HTML porque la fuente no publica API (ENARGAS, ENRE, surtidores YPF, CAC m2). Más frágil — romper si cambia el markup. Todavía no hay ninguno implementado.
- **`auto-llm`** — hay que leer PDFs o páginas con layout complejo (AFIP Monotributo, ANSES jubilaciones, ley de indemnizaciones). Vamos a resolverlo con un agente Claude + WebSearch/WebFetch. Todavía no hay ninguno implementado.
